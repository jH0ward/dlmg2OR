...
//  }
  ...
  //DaoCart_vec.push_back(shared_from_this()->D_subset_helper(Da_,Ca_,"CartAO"));
  //SharedMatrix DaoCart= SharedMatrix(new Matrix(shared_from_this()->D_subset_helper(Da_,Ca_,"CartAO")));
  // changing daocart to be the AO basis, but not worrying about chaning the name of
  // the variable yet
  ...
  long double tot=0.;
//#pragma omp parallel for reduction(+:tot) schedule(static,1) num_threads(thread_var)
...
void RHFDLMG2B::form_H()
{
  outfile->Printf("I'm in my own form_H\n");
  outfile->Printf("attempt number is %d\n",attempt_number_);
  RHF::form_H();
...
void RHFDLMG2B::form_G()
{
    ...
    calc_V_eff3();
    //  changing v eff to be half its value and scaling by 2 to mimic J changes
    G_->axpy(2.0,V_eff_);
    J_->copy(V_eff_);
...
void RHFDLMG2B::calc_V_sol()
{
  ...
  std::cout << "The espao file is gonna be " << std::getenv("TMPDIR")+std::string("/espao");
  ...
  std::cout << "Calling density block\n";
  density_block();
  // temporary calling eps
  //std::cout  << "Calling calc_eps_symm\n" << std::flush;
  //calc_eps_symm();
  // temporary call old dense
  //std::cout << "Calling old density grid code\n" << std::flush;
  //calc_dens_grid();
  std::cout << "Done with old dense code\n";
  //exit(0);
  ...
  // Going to print pot around the middle for debugging
  int myg=81+nx_*81+nx_*ny_*81;
  std::cout << "Printing pot[81][81][81] before solver\n";
  std::cout << pot_sol_[myg] << "\n";

   DL_MG_SOLVER(narray,&pot_sol_[0], &gmat_[0],&order,&rptu,
       &eps_full_[0],&eps_half_[0],&tol_val);

  std::cout << "Printing pot[81][81][81] after solver\n" << std::flush;
  std::cout << pot_sol_[myg] << "\n" << std::flush;
...
void RHFDLMG2B::compute_esp_ao()
{
    ...
    SharedMatrix Dtot = shared_from_this()->D_subset_helper(shared_from_this()->Da(), shared_from_this()->Ca(), "AO");
    Dtot->scale(2.0);
    std::cout << "Opening binary file for writing espao\n";
